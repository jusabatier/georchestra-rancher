...
if [ -e "/var/lib/georchestra/tc-cadastrapp/server.xml" ] ; then
  cp /var/lib/georchestra/tc-cadastrapp/server.xml /usr/local/tomcat/conf/
  chmod a+r /usr/local/tomcat/conf/server.xml
fi
